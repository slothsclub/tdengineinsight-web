...
<template>
<a-space class="window-clause-container">
  ...
  <a-input-group compact v-if="windowClause.type === 'interval'">
    ...
    <a-input-number v-model:value="windowClause.slidingVal" placeholder="" v-if="windowClause.intervalMode === 'sliding'" />
    <a-select v-model:value="windowClause.slidingValUnit" v-if="windowClause.intervalMode === 'sliding'">
      <a-select-option value="s">{{ $t('common.seconds') }}</a-select-option>
      <a-select-option value="m">{{ $t('common.minutes') }}</a-select-option>
      <a-select-option value="h">{{ $t('common.hours') }}</a-select-option>
    </a-select>
  ...
  <a-input-group compact v-if="windowClause.type === 'state'">
    ...
    <a-select v-model:value="windowClause.stateColumn">
      <a-select-option :value="col.colName" v-for="col in columnStore.filteredColumns" :disabled="col.colName === 'ts'">{{ col.colName }}</a-select-option>
    </a-select>
  </a-input-group>
